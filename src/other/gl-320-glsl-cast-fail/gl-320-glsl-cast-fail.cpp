...
Shader castFailShader("glsl-cast-fail");
...
{
	castFailShader.init();
	castFailShader.attach(GL_VERTEX_SHADER, vs_shader_code);
	castFailShader.attach(GL_FRAGMENT_SHADER, fs_shader_code);
	castFailShader.link();
	castFailShader.use();
	program = castFailShader.program; 
