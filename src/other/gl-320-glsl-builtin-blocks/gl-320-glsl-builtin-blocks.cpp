...
const char *vs_shader_code("glsl-builtin-blocks.vert");
const char *fs_shader_code("glsl-builtin-blocks.frag");
GLuint program = 0, vao = 0, texture;
GLuint mvp_loc = 0;
GLuint uTransform_loc = 0, uMaterial_loc;
GLuint uboTransform, uboMaterial;
Shader builtinBlockShader("glsl-builtin-blocks");
...
{
	builtinBlockShader.init();
	builtinBlockShader.attach(GL_VERTEX_SHADER, vs_shader_code);
	builtinBlockShader.attach(GL_FRAGMENT_SHADER, fs_shader_code);
	builtinBlockShader.link();
	builtinBlockShader.use();
	program = builtinBlockShader.program; 
...
void init_texture()
{
	    gli::texture2D Texture( gli::load_dds("../media/kueken1-bgr8.dds") );
